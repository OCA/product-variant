...
class TestPurchaseOrderVariantMgmt(common.SavepointCase):
    ...
    def setUpClass(cls):
        ...
        order = cls.env["purchase.order"].new(
            {"partner_id": cls.partner.id, "company_id": 1}
        )
    ...
    def test_add_variants(self):
        ...
        self.assertEqual(
            len(self.order.order_line), 4, "There should be 4 lines in the sale order"
        )
    ...
    def test_modify_variants(self):
        ...
        Wizard2 = self.Wizard.with_context(
            ...
            active_id=order_line1.id,
            active_ids=order_line1.ids,
        )
        wizard = Wizard2.create({})
        wizard._onchange_product_tmpl_id()
        self.assertEqual(
            len(wizard.variant_line_ids.filtered("product_uom_qty")),
            2,
